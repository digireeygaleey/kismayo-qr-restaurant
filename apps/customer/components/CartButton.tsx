...
export default function CartButton({ slug, table }: { slug: string; table: string }) {
  ...
  return (
    <Link
      ...
      className="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-3 rounded-2xl bg-emerald-600 px-6 py-3.5 text-white shadow-lg transition-all duration-200 hover:bg-emerald-700 hover:shadow-xl active:scale-[0.98]"
    >
      <span className="relative">
        ...
        <span className="absolute -right-2 -top-2 flex h-4.5 w-4.5 min-w-[18px] items-center justify-center rounded-full bg-white px-1 text-[10px] font-bold text-emerald-600">
          {count}
        </span>
      </span>
