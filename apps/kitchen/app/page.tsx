...
const BUTTON_STYLES: Record<string, string> = {
  PREPARING: 'bg-amber-500 hover:bg-amber-400 text-amber-950',
  READY: 'bg-green-500 hover:bg-green-400 text-green-950',
  SERVED: 'bg-blue-500 hover:bg-blue-400 text-blue-950',
};

function getTimerColor(elapsedSeconds: number, prepMinutes?: number | null): string {
  if (!prepMinutes) {
    if (elapsedSeconds < 180) return 'text-green-400';
    if (elapsedSeconds < 300) return 'text-amber-400';
    return 'text-red-400';
  }
...
function OrderTicket({
  ...
  return (
    <div className={`overflow-hidden rounded-xl border bg-surface-900 shadow-ticket ${config?.accent || 'border-white/10'}`}>
      <div className={`flex items-center justify-between px-4 py-2.5 ${config?.header || 'bg-white/5'}`}>
      ...
      <div className="px-4 py-3">
        <p className="mb-2 text-xs font-medium text-white/50">{order.customerName}</p>
        <div className="space-y-1.5">
          {order.items?.map((item) => (
            <div
              key={item.id}
              className={`flex items-baseline gap-2 text-sm ${expoMode ? 'cursor-pointer rounded-lg px-2 py-1 transition-colors hover:bg-white/5' : ''}`}
              onClick={() => expoMode && onBumpItem(order.id, item.id)}
            >
              <span className="font-bold text-white">{item.quantity}x</span>
              <span className="text-white/70">{item.menuItem?.name ?? 'Item'}</span>
              {item.specialInstructions && (
                <span className="text-[10px] text-amber-400/60">({item.specialInstructions})</span>
              )}
            </div>
          ))}
        </div>
        {order.notes && (
          <div className="mt-3 rounded-lg bg-amber-500/10 px-3 py-2">
            <p className="text-xs text-amber-400/80">{order.notes}</p>
          </div>
        )}
      </div>
      {nextStatus && (
        <div className="px-4 pb-4">
          <button
            ...
            className={`w-full rounded-lg py-2.5 text-sm font-bold transition-all active:scale-[0.98] ${BUTTON_STYLES[nextStatus] || 'bg-white/10 hover:bg-white/20 text-white'}`}
          >
...
export default function KitchenPage() {
      ...
      <div className="flex min-h-screen items-center justify-center bg-surface-950 px-4">
        <div className="w-full max-w-sm space-y-6">
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-white/40">Sign in to start receiving orders</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-white/20 focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/20"
            ...
            {loginError && (
              <div className="rounded-xl bg-red-500/10 px-4 py-3 text-sm text-red-400">{loginError}</div>
            )}
            <button
              type="submit"
              disabled={loginLoading}
              className="w-full rounded-xl bg-amber-500 py-3 font-bold text-amber-950 transition-all hover:bg-amber-400 disabled:opacity-40 active:scale-[0.98]"
            >
  ...
  return (
    <div className="flex min-h-screen flex-col bg-surface-950">
      <header className="flex items-center justify-between border-b border-white/5 bg-surface-900/80 px-6 py-3 backdrop-blur">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-lg border border-white/10 px-3 py-1.5 text-xs font-medium text-white/50 transition-colors hover:bg-white/5 hover:text-white/70"
          >
      ...
      {allDayCounts.length > 0 && (
        <div className="border-b border-white/5 bg-surface-900/40 px-6 py-2">
          <div className="flex items-center gap-4 overflow-x-auto scrollbar-none">
            <span className="shrink-0 text-[10px] font-bold tracking-wider text-white/30">ALL-DAY</span>
            {allDayCounts.map(([name, count]) => (
              <div key={name} className="shrink-0 flex items-center gap-1.5 rounded-full bg-white/5 px-3 py-1">
      ...
      <div className="grid flex-1 grid-cols-3 gap-4 p-4 overflow-auto">
        {[
          ...
          <div key={key} className="flex flex-col">
            ...
            <div className="flex-1 space-y-3 overflow-auto rounded-b-xl border border-t-0 border-white/5 bg-surface-950 p-3">
              {colOrders.length === 0 && (
